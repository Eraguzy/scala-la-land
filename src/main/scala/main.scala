...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val system = ActorSystem(Behaviors.setup[Nothing] { ctx =>
      ...
      val alice = ctx.spawn(WalletActor("alice_key", "alice_key", 500, mempool), "Alice")
      val bob = ctx.spawn(WalletActor("bob_key", "bob_key", 100, mempool), "Bob")
      val charlie = ctx.spawn(WalletActor("charlie_key", "charlie_key", 50, mempool), "Charlie")
      ...
      alice ! Wallet.CreateTx("bob_pk", 10) // Fee par défaut (ex: 10)


      // B. Transaction "Prioritaire" : Gros montant et gros frais
      alice ! Wallet.CreateTx("charlie_pk", 100) // On imagine ici que tu as ajouté un champ fee ou qu'il est géré
      ...
      bob ! Wallet.CreateTx("alice_pk", 150)

      // D. Flux croisé : Charlie envoie à Alice pendant que le Validator travaille
      ctx.scheduleOnce(1.second, charlie, Wallet.CreateTx("alice_pk", 20))
